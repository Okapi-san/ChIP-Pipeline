...
while [[ $end_of_file == 0 ]]; do
	...
	echo $(echo $(grep $var Output_sorted_metadata.txt | cut -d';' -f 4-4 | tr ' ' '\n' | paste -sd+ | bc) / $(grep $var Output_sorted_metadata.txt | wc -l  | awk '{ print $1 }') | bc -l | sed 's/^./0./g') >> $var.target
done < "targets"
cat *.target | sort -r -k 2 > Summary.txt
rm *.target
